...
| `confirmUrlBase` | *derived* — the public distribution plus `/confirm` | Base URL used in double-opt-in confirmation links. Set only to override, when that route is served somewhere other than this stack's public distribution. It used to fall back to `https://your-site.example/confirm`, which put a dead link in every confirmation email while signup returned 200 and the mail sent (#294). |
| `preferencesUrlBase` | *derived* — the public distribution plus `/preferences` | Base URL used in subscription-management emails. Set only to override. Same story as `confirmUrlBase`, on the surface subscribers use to leave — and it was the one that had never been set at all, so every emailed management link was dead until 2026-09-23. |
...
> **`confirmUrlBase` and `preferencesUrlBase` are DERIVED** (#294). They are the
> links addressium mails subscribers, and the stack builds the distribution that
> serves those routes — so it resolves both from `PublicSiteDist` at synth, as an
> `Fn::Join` that cannot drift from the live site. A first deploy needs nothing.
>
> Set them in `infra/cdk/addressium.config.json` only to **override**, when those
> routes are served somewhere else:
>
> ```json
...
> and were readable only from `cdk` context — so they had to be passed as `-c` on
> *every* deploy, and forgetting once silently reverted a working stack: signup
> returned 200, the mail sent, every link in it was dead, and no alarm, log or
> exit code said so. `npm run deploy` and CI now both reject that placeholder
> string if it is ever pasted back in.
>
> `-c confirmUrlBase=…` still overrides for a one-off deploy, but note
