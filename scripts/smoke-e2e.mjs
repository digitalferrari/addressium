 ...
 *   SMOKE_ADMIN_TOKEN=<Cognito-access-token> node scripts/smoke-e2e.mjs
 *
 * ## Read this before running it
 *
 * The stack has delivered controlled mail, but this suite has not yet proved its
 * full public/admin/event path. It is written from the deployed API contract;
 * failures are verification findings and must be fixed rather than waived.
 *
 * ## Safety
 *
...
const ADMIN_TOKEN = need("SMOKE_ADMIN_TOKEN");

const stamp = process.env.SMOKE_RUN_ID ?? String(process.hrtime.bigint());
// Provisioning derives the org id from its name. Keeping the two equal prevents
// every following step from addressing a nonexistent hard-coded `smoketest` org.
const ORG = process.env.SMOKE_ORG ?? `smoke-${stamp}`;
const LIST = "smoke";
const SIMULATORS = ["bounce@simulator.amazonses.com", "complaint@simulator.amazonses.com"];
...
async function main() {
  ...
  const readLink = async (kind, configured) => {
    if (configured) return configured;
    if (!stdin.isTTY) {
      throw new Error(`missing ${kind}; set it when running non-interactively`);
    }
    return (await prompt.question(`Open the ${kind} sent to ${RECIPIENT}, then paste its URL: `)).trim();
  };
  ...
  await step("provision a dev org allowlisted to exactly one address", async () => {
    const r = await api("POST", "/orgs", {
      ...
      environment: "dev",
      // The simulator addresses are AWS-owned and test only bounce/complaint
      // behavior; they are still explicitly allowlisted so dev remains
      // fail-closed for every other recipient.
      devAllowlist: [RECIPIENT, ...SIMULATORS],
      magicLinks: false,
    }, ADMIN_TOKEN);
  ...
  let confirmUrl;
  await step("signup delivers a double-opt-in confirmation email", async () => {
    const r = await api("POST", "/signup", { orgId: ORG, listId: LIST, email: RECIPIENT });
    assert(r.status < 300, `POST /signup → ${r.status}`);
    assert(r.body?.subscriberId, "signup response did not identify its pending subscriber");
    // No inbound SES receipt rule belongs in a delivery proof: it would require
    // making this account receive mail for the recipient's domain. The controlled
    // mailbox is the authoritative proof that the person received this message.
    confirmUrl = await readLink("SMOKE_CONFIRM_URL", process.env.SMOKE_CONFIRM_URL);
    assert(confirmUrl.startsWith("https://"), "confirmation URL must be HTTPS");
  });

  await step("following the confirm link confirms the subscription", async () => {
    const link = new URL(confirmUrl);
    const r = await api("GET", `${link.pathname}${link.search}`);
  ...
  await step("a campaign sends, and its headers are RFC 8058 correct (#178)", async () => {
    ...
    // Paste the visible unsubscribe link from the controlled delivery (or pass
    // SMOKE_UNSUBSCRIBE_URL in an automated mailbox harness). Posting it below
    // exercises the RFC 8058 one-click endpoint without an unrelated inbound-S3
    // requirement in the Addressium account.
    unsubUrl = await readLink("SMOKE_UNSUBSCRIBE_URL", process.env.SMOKE_UNSUBSCRIBE_URL);
    assert(unsubUrl.startsWith("https://") && /token=/.test(unsubUrl), "unsubscribe URL is not a signed HTTPS link");
  });

  await step("one-click unsubscribe works and suppresses", async () => {
    const link = new URL(unsubUrl);
    const res = await api("POST", `${link.pathname}${link.search}`);
    assert(res.status < 300, `POST unsubscribe → ${res.status}`);
    const subs = await api("GET", `/orgs/${ORG}/suppressions`, undefined, ADMIN_TOKEN);
    assert(
  ...
  await step("SES events reach DynamoDB and move the counters (#208, #184)", async () => {
    ...
    const report = await until("engagement events to land", async () => {
      const r = await api("GET", `/orgs/${ORG}/campaigns/smoke-${stamp}/report`, undefined, ADMIN_TOKEN);
  ...
  for (const kind of ["bounce", "complaint"]) {
    await step(`a simulated ${kind} suppresses and can trip the halt gate (#165)`, async () => {
      ...
      await until(`${kind} suppression`, async () => {
        const s = await api("GET", `/orgs/${ORG}/suppressions`, undefined, ADMIN_TOKEN);
