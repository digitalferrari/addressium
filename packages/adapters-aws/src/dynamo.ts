...
/** Backoff between batch-write retries (#240). */
...
export class DynamoStores implements Stores {
  ...
  /**
   * Write many items in as few round trips as DynamoDB allows (#240).
   *
   * Three constraints, each of which produces a different bug if ignored:
   *
   *  1. **25 items per request.** Over that, DynamoDB rejects the whole batch
   *     with a ValidationException — not a partial write.
   *  2. **No duplicate keys within one request.** Also a whole-batch rejection.
   *     A suppression list can legitimately name the same address twice (the
   *     provider paginates, and a page boundary can repeat under concurrent
   *     modification), so the caller cannot be relied on to have deduped. Last
   *     one wins, matching `PutCommand` semantics for a single write and the
   *     memory store's `addMany`.
   *  3. **`UnprocessedItems` is normal, not an error.** A throttled partition
   *     returns 200 with the leftovers in the response body. Treating that as
   *     success silently drops suppression entries — the exact failure this
   *     whole path exists to prevent — so they are retried with backoff, and a
   *     batch that will not drain throws rather than reporting a write it did
   *     not make.
   */
  private async putMany(items: Item<unknown>[]): Promise<void> {
    const CHUNK = 25;
    const MAX_ATTEMPTS = 6;
    for (let i = 0; i < items.length; i += CHUNK) {
      const byKey = new Map<string, Item<unknown>>();
      for (const it of items.slice(i, i + CHUNK)) byKey.set(`${it.pk}\x00${it.sk}`, it);
      let pending = [...byKey.values()].map((Item) => ({ PutRequest: { Item } }));
      for (let attempt = 0; pending.length > 0; attempt++) {
        if (attempt >= MAX_ATTEMPTS) {
          throw new Error(
            `batch write did not drain after ${MAX_ATTEMPTS} attempts; ${pending.length} item(s) unwritten`,
          );
        }
  ...
  /**
   * The item for one suppression entry — shared by `add` and `addMany` so the
   * bulk path cannot drift from the single-write path. Global entries
   * (bounces/complaints) live in a cross-org partition (§4.13); the email is
   * lower-cased into the key because that is what `isSuppressed` looks up, and a
   * mixed-case address written raw would be invisible to the gate that matters.
   */
  private static suppressionItem(e: SuppressionEntry): Item<SuppressionEntry> {
  ...
  suppression: SuppressionStore = {
    ...
    add: (e) => this.put(DynamoStores.suppressionItem(e)),
    addMany: (entries) => this.putMany(entries.map((e) => DynamoStores.suppressionItem(e))),
  ...
  private async appendEvent(e: EngagementEvent): Promise<void> {
    ...
    const bumpCounter = {
      Update: {
        ...
        // ONE path only. Setting `data.counters` and `data.counters.<field>` in
        // the same expression is a ValidationException on real DynamoDB —
        //   "Two document paths overlap with each other ... path one:
        //    [data, counters], path two: [data, counters, sent]"
        // — and because this Update rides in a TransactWrite with the event Put,
        // the whole transaction is rejected BEFORE it runs. That is not a
        // cancellation, so none of the CancellationReasons recovery below sees
        // it: every send, open, click and bounce failed to record while the mail
        // itself went out fine. The memory store accepts the overlapping form,
        // which is why the suite stayed green.
        //
        // `if_not_exists` on the leaf still covers a campaign whose map predates
        // counters; the map itself is created when the campaign row is written
        // (see `campaigns.put`), so there is nothing to lazily create here.
        UpdateExpression: "SET #c.#cnt.#f = if_not_exists(#c.#cnt.#f, :zero) + :one",
